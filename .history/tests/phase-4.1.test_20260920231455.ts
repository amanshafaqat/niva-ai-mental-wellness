...
async function runTests() {
  ...
  assert.strictEqual(ticketResponse.model, 'gemini-live-2.5-flash-preview', 'Voice model must match the supported live model');
  assert.strictEqual(ticketResponse.voiceName, 'NIVA Voice', 'Voice name must be NIVA Voice');
