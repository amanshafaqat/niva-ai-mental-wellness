...
export const DashboardView: React.FC<DashboardViewProps> = ({ onOpenCrisisResources }) => {
  ...
  const [activeTab, setActiveTab] = useState<'session' | 'guardians' | 'activity' | 'admin' | 'profile' | 'privacy'>('session');
  ...
  return (
    <div id="authenticated-dashboard" className="py-8">
      <div className="mx-auto max-w-6xl px-4 sm:px-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="mt-1 flex items-center gap-3 text-xs text-stone-500">
                  ...
                  <span>•</span>
                  <span>{user.status || 'Active'} account status</span>
                </div>
              </div>
            </div>
        ...
        <div className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4 text-xs text-emerald-950 flex items-start gap-3">
          <Sparkles className="h-4 w-4 text-emerald-700 shrink-0 mt-0.5" />
          <div>
            Your wellness space is ready. Use the tools below to continue your care journey, manage support preferences, and access trusted crisis resources when needed.
          </div>
        </div>
