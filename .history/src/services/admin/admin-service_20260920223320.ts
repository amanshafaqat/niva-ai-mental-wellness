...
export async function getAdminUsersList(
  ...
  const requestedRole = options.role ? normalizeRoleValue(String(options.role)) : null;
  const requestedStatus = options.status ? normalizeStatusValue(String(options.status)) : null;
  ...
  if (requestedRole) {
    filtered = filtered.filter((u) => normalizeRoleValue(String(u.role || '')) === requestedRole);
  }
  if (requestedStatus) {
    filtered = filtered.filter((u) => normalizeStatusValue(String(u.status || '')) === requestedStatus);
  }
  if (options.search && options.search.trim().length > 0) {
