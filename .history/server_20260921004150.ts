...
export function getConfiguredInitialAdminEmail(): string | null {
...
export async function ensureInitialAdminBootstrap(): Promise<void> {
  ...
  const existingMemoryUser = memoryUsers.get(configuredEmail);
  const memoryUser = {
    id: existingMemoryUser?.id || `usr_${crypto.randomBytes(8).toString('hex')}`,
    email: adminUser.email,
    name: adminUser.name || 'NIVA Platform Administrator',
    avatarUrl: existingMemoryUser?.avatarUrl || null,
    role: Role.ADMIN,
    status: 'ACTIVE',
    isActive: true,
    createdAt: existingMemoryUser?.createdAt || adminUser.createdAt.toISOString(),
    updatedAt: adminUser.updatedAt.toISOString(),
    lastLoginAt: existingMemoryUser?.lastLoginAt || null,
  };

  memoryUsers.set(configuredEmail, memoryUser);
}
...
const getHealthResponse = async () => {
  ...
  const clientId = process.env.GOOGLE_CLIENT_ID;
  const googleReady = Boolean(clientId && !isPlaceholderConfigValue(clientId) && clientId.trim().length > 0);
...
const handleGoogleRedirect = (req: express.Request, res: express.Response) => {
  ...
  if (!clientId || isPlaceholderConfigValue(clientId)) {
...
const handleGoogleCallback = async (req: express.Request, res: express.Response) => {
  ...
  if (!clientId || !clientSecret || isPlaceholderConfigValue(clientId) || isPlaceholderConfigValue(clientSecret)) {
    logAuditEvent(
      AuditAction.LOGIN_FAILURE,
      null,
      'OAuth',
...
const handlePostConversationMessage = async (req: express.Request, res: express.Response) => {
  ...
  if (!safetyCheck.isSafe && safetyCheck.blockedResponse) {
    ...
    if (!isConfiguredGeminiApiKey(process.env.GEMINI_API_KEY)) {
      return res.status(503).json({
        success: false,
        statusCode: 503,
        errorCode: 'AI_SERVICE_UNAVAILABLE',
        message: 'NIVA AI is unavailable because GEMINI_API_KEY is not configured on the server.',
      });
    }
    ...
    try {
      ...
      console.error('AI reply failed while processing message:', err?.message || err);
      return res.status(503).json({
        success: false,
        statusCode: 503,
        errorCode: 'AI_SERVICE_UNAVAILABLE',
...
const handleCreateVoiceTicket = async (req: express.Request, res: express.Response) => {
  ...
  if (!isConfiguredGeminiApiKey(process.env.GEMINI_API_KEY)) {
    return res.status(503).json({
      ...
      statusCode: 503,
      errorCode: 'AI_SERVICE_UNAVAILABLE',
      message: 'NIVA voice is unavailable because GEMINI_API_KEY is not configured on the server.',
    });
  }
...
async function startServer() {
  await ensureInitialAdminBootstrap();
